...
async function runSinglePostMortem(interaction, trade) {
  ...
  try {
    ...
    const markdown = [
      `# Post-Mortem Analysis — Trade #${trade.id} ${trade.asset}`,
      '',
      `| Field | Value |`,
      `|-------|-------|`,
      `| Asset | ${trade.asset} |`,
      ...
      '',
      '---',
      `*Generated by Journal Node | Payment: ${paymentTx.txUrl}*`,
    ].join('\n');

    const fileName = `postmortem_${trade.asset}_${trade.id}.md`;
    const file = new AttachmentBuilder(Buffer.from(markdown, 'utf-8'), { name: fileName });
    ...
    const resultEmbed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        'Full AI post-mortem analysis attached below.'
      );

    await interaction.editReply({ embeds: [resultEmbed], files: [file], components: [] });
  } catch (err) {
...
async function runTimeframePostMortem(interaction, trades, startDate, endDate) {
  ...
  try {
    ...
    const markdown = [
      ...
      '',
      ...trades.map((t, i) => [
        `### ${i + 1}. #${t.id} ${t.asset} ${t.direction} — ${t.outcome}`,
        `- Entry: ${t.entry} | Exit: ${t.exitPrice || 'N/A'} | Target: ${t.target} | Stop Loss: ${t.stopLoss || 'N/A'}`,
        `- Timeframe: ${t.timeframe} | ${formatDate(t.createdAt)} — ${formatDate(t.closedAt)}`,
        '',
      ].join('\n')),
      '---',
      '',
    ...
    const dateSuffix = label.replace(/[^a-zA-Z0-9]/g, '_');
    const fileName = `timeframe_analysis_${dateSuffix}.md`;
    const file = new AttachmentBuilder(Buffer.from(markdown, 'utf-8'), { name: fileName });
    ...
    const resultEmbed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        `**Payment:** [View TX](${paymentTx.txUrl}) (${LLM_FEE_PFT} PFT)\n\n` +
        'Full AI timeframe analysis attached below.'
      );
